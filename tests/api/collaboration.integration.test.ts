...
async function fixture() {
  ...
  await c.user.createMany({
    data: ['alice', 'bob', 'outsider'].map((id) => ({
      id,
      name: id,
  ...
  await c.agentRun.create({
    data: {
      ...
      kind: 'TOOL',
      status: 'SUCCEEDED',
      intent: { name: 'read_file', relativePath: 'a.txt' },
      createdAt: now,
      startedAt: now,
      finishedAt: now,
      updatedAt: now
  ...
  await c.artifact.create({
    data: {
      id: 'artifact',
      projectId: 'project',
      taskId: 'task',
      agentRunId: 'artifact-run',
      sourceToolCallId: 'artifact-call',
      type: 'FILE',
...
describe('Demo collaboration API', () => {
  beforeEach(clean);
  afterAll(async () => {
    if (database) await clean();
    await database?.$disconnect();
  });
  it('creates scoped conversations and idempotent Session-authored messages without cross-organization leakage', async () => {
    await fixture();
    const app = await createApp({ prisma: db() }),
      alice = await auth('alice'),
      bob = await auth('bob'),
    ...
    const id = conversation.json().conversationId;
    expect(
      (
        await app.inject({ method: 'GET', url: '/conversations', headers: bob })
      ).json().items
    ...
    const headers = { ...alice, 'idempotency-key': 'message-key' };
    const first = await app.inject({
      method: 'POST',
      url: `/conversations/${id}/messages`,
      headers,
      payload: { content: 'hello', authorUserId: 'forged' }
    });
    ...
    const good = await app.inject({
      ...
      url: `/conversations/${id}/messages`,
      headers,
      payload: { content: 'hello', mentionedUserIds: ['bob'] }
    });
    ...
    const conflict = await app.inject({
      ...
      url: `/conversations/${id}/messages`,
      headers,
      payload: { content: 'changed', mentionedUserIds: ['bob'] }
    });
    expect(good.statusCode).toBe(200);
    expect(replay.json().messageId).toBe(good.json().messageId);
    expect(conflict.statusCode).toBe(409);
    expect(await db().message.count()).toBe(1);
    await app.close();
  });
  it('does not derive mention actions from a private AI thread and rejects mismatched project scopes', async () => {
    await fixture();
    const app = await createApp({ prisma: db() }),
      alice = await auth('alice'),
      bob = await auth('bob');
    ...
    expect(privateConversation.statusCode).toBe(200);
    const message = await app.inject({
      method: 'POST',
      url: `/conversations/${privateConversation.json().conversationId}/messages`,
      headers: { ...alice, 'idempotency-key': 'private-mention' },
      payload: { content: 'private', mentionedUserIds: ['bob'] }
    });
    ...
    expect(
      actions
        .json()
        .some(
          (item: { actionItemId: string }) =>
            item.actionItemId === `mention:${message.json().messageId}`
        )
    ).toBe(false);
    await app.close();
  });
  it('enforces notification and reminder ownership while returning derived library swarm and action read models', async () => {
    await fixture();
    const app = await createApp({ prisma: db() }),
      alice = await auth('alice'),
      bob = await auth('bob'),
    ...
    expect(
      (
        await app.inject({
          method: 'PATCH',
          url: `/reminders/${created.json().reminderId}`,
          headers: bob,
    ...
    expect(
      (
        await app.inject({
          method: 'GET',
          url: '/library-items?type=ARTIFACT',
          headers: alice
        })
      ).json().items
    ).toEqual([expect.objectContaining({ sourceObjectId: 'artifact' })]);
    expect(
      (
        await app.inject({
          method: 'GET',
          url: '/swarm-events?scopeType=PROJECT&scopeId=project',
          headers: bob
        })
      ).json().items
    ).toHaveLength(1);
