...
async function artifact(
  ...
  const run = (
    await app.inject({
      method: 'POST',
      url: `/tasks/${taskId}/agent-runs`,
      payload: { name: 'read_file', relativePath: path }
    })
  ).json();
  await app.inject({
    method: 'POST',
    ...
    payload: {
      ...
      status: 'SUCCEEDED',
      metadata: { relativePath: path, size: 1, encoding: 'utf-8', sha256: hash }
    }
  });
  return (
    await app.inject({
...
describe('Result Candidate API', () => {
  ...
  it('creates canonical Candidate Results idempotently without changing Artifact, Task, or AgentRun', async () => {
    ...
    const canonicalIds = [a.id, b.id].sort((left: string, right: string) =>
      left < right ? -1 : left > right ? 1 : 0
    );
    const artifactsBefore = await db().artifact.findMany({
      where: { taskId: task.id },
      orderBy: { id: 'asc' }
    });
    const runsBefore = await db().agentRun.findMany({
      where: { taskId: task.id },
      orderBy: { id: 'asc' },
    ...
    const first = await app.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
      headers: { 'idempotency-key': key('1') },
      payload: { artifactIds: [b.id, a.id] }
    });
    expect(first.statusCode).toBe(201);
    expect(first.json()).toMatchObject({
      status: 'CANDIDATE',
      createdByUserId: 'dev-user',
      artifactIds: canonicalIds
    });
    const retry = await app.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
      headers: { 'idempotency-key': key('1') },
      payload: { artifactIds: canonicalIds }
    });
    expect(retry.statusCode).toBe(200);
    expect(retry.json()).toEqual(first.json());
    expect(
      (
        await app.inject({ method: 'GET', url: `/results/${first.json().id}` })
      ).json()
    ).toEqual(first.json());
    expect(
    ...
    const sameKeyDifferentSet = await app.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
      headers: { 'idempotency-key': key('1') },
      payload: { artifactIds: [a.id] }
    });
    expect(sameKeyDifferentSet).toMatchObject({ statusCode: 409 });
    expect(sameKeyDifferentSet.json()).toMatchObject({
      error: { code: 'IDEMPOTENCY_KEY_CONFLICT' }
    });
    const differentKeySameSet = await app.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
  ...
  it('hides invalid Artifact scope and rejects invalid Candidate input', async () => {
    ...
    const secondProjectTask = (
      await app.inject({
        method: 'POST',
        url: `/projects/${secondProject.id}/tasks`,
    ...
    expect(
      (await request({ artifactIds: [foreign.id, foreign.id] })).statusCode
    ).toBe(400);
    expect((await request({ artifactIds: [foreign.id] })).statusCode).toBe(404);
    expect(
      (await request({ artifactIds: [crossProject.id] }, key('4'))).statusCode
    ).toBe(404);
    expect(
      (await request({ artifactIds: ['missing-artifact'] }, key('5')))
        .statusCode
    ).toBe(404);
    expect(
      (
    ...
    expect(
      (
        await app.inject({
          method: 'POST',
          url: `/tasks/${task.id}/results`,
          payload: { artifactIds: [foreign.id] }
        })
      ).statusCode
    ).toBe(400);
    expect(
    ...
    expect(
      (
        await app.inject({
          method: 'POST',
          url: `/tasks/${task.id}/results`,
          headers: { 'idempotency-key': 'x'.repeat(100) },
          payload: { artifactIds: [foreign.id] }
        })
      ).statusCode
    ).toBe(400);
  ...
  it('hides Result creation and reads when membership is absent or revoked', async () => {
    ...
    const created = await app.inject({
      ...
      payload: { artifactIds: [registered.id] }
    });
    expect(created.statusCode).toBe(201);
    const outsider = await createApp({
      prisma: db(),
      identityProvider: new DevIdentityProvider({ id: 'result-outsider' })
    });
    expect(
      (
  ...
  it('makes overlapping Result creation atomic for same and conflicting idempotency requests', async () => {
    ...
    const app = await createApp({
      prisma: wrapPrismaForResultApiTest(primaryClient, {
    ...
    const a = await artifact(app, task.id, 'a.md');
    const b = await artifact(app, task.id, 'b.md', 'b'.repeat(64));
    const sameARequest = app.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
      headers: { 'idempotency-key': key('10') },
      payload: { artifactIds: [a.id, b.id] }
    });
    const sameApp = await createApp({
      prisma: wrapPrismaForResultApiTest(secondaryClient, {
        beforeResultCreate: () => sameGate.arriveAndWait()
      }),
      identityProvider: new DevIdentityProvider()
    });
    const sameBRequest = sameApp.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
      headers: { 'idempotency-key': key('10') },
      payload: { artifactIds: [a.id, b.id] }
    ...
    expect(
      await db().resultArtifact.count({ where: { resultId: sameA.json().id } })
    ).toBe(2);
    ...
    const left = conflictApp.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
      headers: { 'idempotency-key': key('11') },
      payload: { artifactIds: [a.id] }
    });
    const right = conflictApp.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
      headers: { 'idempotency-key': key('11') },
      payload: { artifactIds: [b.id] }
    });
    ...
    const winner = responses
      ...
      .json();
    const links = await db().resultArtifact.findMany({
      where: { resultId: winner.id },
      orderBy: { artifactId: 'asc' }
    });
    expect(links.map((link) => link.artifactId)).toEqual(winner.artifactIds);
    expect(
      await db().result.count({ where: { idempotencyKey: key('11') } })
    ).toBe(1);
  ...
  it('coordinates submission and human ACCEPT with the owning Task', async () => {
    ...
    const dependent = (
      await app.inject({
        method: 'POST',
        url: `/projects/${project.id}/tasks`,
        payload: { title: 'Dependent', dependencyIds: [task.id] }
      })
    ).json();
    const registered = await artifact(app, task.id, 'a.md');
    const created = await app.inject({
      method: 'POST',
      url: `/tasks/${task.id}/results`,
      headers: { 'idempotency-key': key('12') },
      payload: { artifactIds: [registered.id] }
    });
    ...
    const submitted = await app.inject({
      method: 'POST',
      url: `/results/${result.id}/submit-review`,
      payload: {}
    });
    expect(submitted.json()).toMatchObject({
      status: 'HUMAN_REVIEW',
      submittedByUserId: 'dev-user'
    });
    const firstSubmittedAt = submitted.json().submittedAt;
    const firstTaskUpdatedAt = (
    ...
    ).toBe(firstSubmittedAt);
    expect(
      (await db().task.findUniqueOrThrow({ where: { id: task.id } })).updatedAt
    ).toEqual(firstTaskUpdatedAt);
    expect(
      (await db().task.findUniqueOrThrow({ where: { id: task.id } })).status
    ...
    const reviewer = await createApp({
      prisma: db(),
      identityProvider: new DevIdentityProvider({
        id: 'reviewer-1',
        name: 'Reviewer'
    ...
    const accepted = await reviewer.inject({
      method: 'POST',
      url: `/results/${result.id}/reviews`,
      payload: { decision: 'ACCEPT', comment: ' looks good ' }
    });
    ...
    const acceptedTaskUpdatedAt = (
      await db().task.findUniqueOrThrow({ where: { id: task.id } })
    ).updatedAt;
    expect(
      (
        await reviewer.inject({
          method: 'POST',
          url: `/results/${result.id}/reviews`,
          payload: { decision: 'ACCEPT', comment: 'looks good' }
        })
    ...
    expect(
      (await db().result.findUniqueOrThrow({ where: { id: result.id } })).status
    ).toBe('ACCEPTED');
    expect(
      (await db().task.findUniqueOrThrow({ where: { id: task.id } })).status
    ).toBe('ACCEPTED');
  ...
  it('returns REWORK Result and Task IN_PROGRESS atomically', async () => {
    ...
    const created = (
      await app.inject({
        method: 'POST',
        url: `/tasks/${task.id}/results`,
        headers: { 'idempotency-key': key('15') },
        payload: { artifactIds: [registered.id] }
      })
    ).json();
    await app.inject({ method: 'POST', url: `/tasks/${task.id}/start` });
    await app.inject({
      method: 'POST',
      url: `/results/${created.id}/submit-review`,
      payload: {}
    });
    await db().user.create({
      data: {
    ...
    await db().projectMember.create({
      data: {
        id: 'member-reviewer-rework',
        projectId: project.id,
        userId: 'reviewer-rework',
        role: 'REVIEWER',
        createdAt: new Date(),
        updatedAt: new Date()
      }
    ...
    const reviewer = await createApp({
      prisma: db(),
      identityProvider: new DevIdentityProvider({ id: 'reviewer-rework' })
    });
    expect(
      (
        await reviewer.inject({
          method: 'POST',
          url: `/results/${created.id}/reviews`,
          payload: { decision: 'REWORK' }
        })
      ).statusCode
    ).toBe(201);
    expect(
      await db().result.findUniqueOrThrow({ where: { id: created.id } })
    ).toMatchObject({ status: 'REWORK' });
    expect(
      await db().task.findUniqueOrThrow({ where: { id: task.id } })
  ...
  it('enforces review role, ownership, hidden scope and terminal conflict boundaries', async () => {
    ...
    const created = (
      await app.inject({
        method: 'POST',
        url: `/tasks/${task.id}/results`,
        headers: { 'idempotency-key': key('13') },
        payload: { artifactIds: [registered.id] }
      })
    ).json();
    await app.inject({ method: 'POST', url: `/tasks/${task.id}/start` });
    for (const [id, role, systemRole] of [
      ['member-1', 'MEMBER', 'EMPLOYEE'],
      ['admin-1', 'MEMBER', 'ADMIN'],
      ['reviewer-2', 'REVIEWER', 'EMPLOYEE']
    ] as const) {
      await db().user.create({
        data: {
          id,
    ...
    expect(
      (
        await app.inject({
          method: 'POST',
          url: `/results/${created.id}/submit-review`,
          payload: { submittedByUserId: 'forged' }
        })
      ).statusCode
    ).toBe(400);
    expect(
    ...
    expect(
      (
        await reviewer.inject({
          method: 'POST',
          url: `/results/${created.id}/reviews`,
          payload: { decision: 'REWORK', reviewerId: 'forged' }
        })
      ).statusCode
    ).toBe(400);
    expect(
      (
        await reviewer.inject({
          method: 'POST',
          url: `/results/${created.id}/reviews`,
          payload: { decision: 'REWORK', comment: 'needs work' }
        })
      ).statusCode
    ).toBe(201);
    expect(
    ...
    await db().projectMember.delete({
      where: {
        projectId_userId: { projectId: project.id, userId: 'reviewer-2' }
      }
    });
    expect(
      (
  ...
  it('makes omitted and blank review comments the same idempotent request', async () => {
    ...
    const result = (
      await app.inject({
        method: 'POST',
        url: `/tasks/${task.id}/results`,
        headers: { 'idempotency-key': key('14') },
        payload: { artifactIds: [registered.id] }
      })
    ).json();
    await app.inject({ method: 'POST', url: `/tasks/${task.id}/start` });
    ...
    await db().projectMember.create({
      data: {
        id: 'member-reviewer-empty',
        projectId: project.id,
        userId: 'reviewer-empty',
        role: 'REVIEWER',
        createdAt: new Date(),
        updatedAt: new Date()
      }
    ...
    const first = await reviewer.inject({
      method: 'POST',
      url: `/results/${result.id}/reviews`,
      payload: { decision: 'ACCEPT' }
    });
    const blank = await reviewer.inject({
      method: 'POST',
      url: `/results/${result.id}/reviews`,
      payload: { decision: 'ACCEPT', comment: '   ' }
    });
    const changed = await reviewer.inject({
      method: 'POST',
      url: `/results/${result.id}/reviews`,
      payload: { decision: 'ACCEPT', comment: 'different' }
    });
    expect(first.statusCode).toBe(201);
    expect(blank.statusCode).toBe(200);
    expect(blank.json().id).toBe(first.json().id);
  ...
  it('rechecks live Organization RESULT REVIEW permission for the same reviewer session', async () => {
    ...
    const project = (
      await app.inject({
        method: 'POST',
        url: '/projects',
        payload: { name: 'Permission review' }
      })
    ).json();
    const task = (
      await app.inject({
    ...
    const result = (
      await app.inject({
        method: 'POST',
        url: `/tasks/${task.id}/results`,
        headers: { 'idempotency-key': key('16') },
        payload: { artifactIds: [registered.id] }
      })
    ).json();
    await app.inject({ method: 'POST', url: `/tasks/${task.id}/start` });
    ...
    await db().projectMember.create({
      data: {
        id: 'permission-reviewer-member',
        projectId: project.id,
        userId: 'permission-reviewer',
        role: 'REVIEWER',
        createdAt: now,
        updatedAt: now
      }
    ...
    await db().organizationMembership.createMany({
      data: [
        {
          id: 'permission-review-owner',
          organizationId: 'permission-review-org',
          userId: 'dev-user',
          role: 'OWNER',
          status: 'ACTIVE',
          createdAt: now,
    ...
    expect(
      (
        await app.inject({
          method: 'PUT',
          url: '/employees/permission-reviewer/permission-overrides',
          payload: {
            scopeType: 'ORGANIZATION',
            scopeId: 'permission-review-org',
            resource: 'RESULT',
            action: 'REVIEW',
            effect: 'DENY'
          }
    ...
    expect(await db().review.count({ where: { resultId: result.id } })).toBe(0);
    expect(
      (await db().result.findUniqueOrThrow({ where: { id: result.id } })).status
    ).toBe('HUMAN_REVIEW');
    expect(
    ...
    const disabledTask = (
      await app.inject({
        method: 'POST',
        url: `/projects/${project.id}/tasks`,
        payload: { title: 'Disabled reviewer' }
      })
    ).json();
    const disabledArtifact = await artifact(
      app,
      disabledTask.id,
      'disabled.md'
    );
    const disabledResult = (
      await app.inject({
        method: 'POST',
        url: `/tasks/${disabledTask.id}/results`,
        headers: { 'idempotency-key': key('17') },
        payload: { artifactIds: [disabledArtifact.id] }
      })
    ).json();
    await app.inject({
      method: 'POST',
      url: `/tasks/${disabledTask.id}/start`
    });
    await app.inject({
      method: 'POST',
      url: `/results/${disabledResult.id}/submit-review`,
      payload: {}
    });
    await db().organizationMembership.update({
      where: { userId: 'permission-reviewer' },
    ...
    expect(
      (
        await reviewer.inject({
          method: 'POST',
          url: `/results/${disabledResult.id}/reviews`,
          payload: { decision: 'ACCEPT' }
        })
      ).statusCode
    ).toBe(403);
    expect(
      await db().review.count({ where: { resultId: disabledResult.id } })
    ).toBe(0);
    await reviewer.close();
    await app.close();
  });
...
function wrapPrismaForResultApiTest(
  prisma: PrismaClient,
  hooks: { beforeResultCreate: () => Promise<void> | void }
): PrismaClient {
...
function boundedGate(parties: number, timeoutMs = 5_000) {
  ...
  const timeout = setTimeout(
    () =>
      fail(
        new Error(
          `timed out waiting for ${parties} Result transaction boundaries`
        )
      ),
    timeoutMs
  );
