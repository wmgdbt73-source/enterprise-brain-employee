...
async function createAccount(
  ...
  await db().account.create({
    data: {
      ...
      userId: id,
      login,
      passwordHash: await encodePassword('DemoPassword!2026'),
      status,
      createdAt: now,
      updatedAt: now
    }
...
describe('production session identity API', () => {
  ...
  it('logs in, resolves /me, revokes only that session, and hides credential material', async () => {
    ...
    expect(login.json()).toMatchObject({ user: { id: 'auth-user' } });
  ...
  it('returns one generic failure for wrong credentials and rejects unauthenticated mutations before they write', async () => {
    ...
    const wrongLogin = await app.inject({
      method: 'POST',
      url: '/auth/login',
      payload: { login: 'missing@example.test', password: 'nope' }
    });
    const wrongPassword = await app.inject({
      method: 'POST',
      url: '/auth/login',
      payload: { login: 'employee@example.test', password: 'nope' }
    });
    expect(wrongLogin.statusCode).toBe(401);
    expect(wrongLogin.json()).toEqual(wrongPassword.json());
    expect(
      (
        await app.inject({
          method: 'POST',
  ...
  it('rejects malformed, expired, revoked, and disabled identities on each protected request', async () => {
    ...
    for (const authorization of [undefined, 'Bearer invalid', 'Basic nope'])
      expect(
        (
          await app.inject({
            method: 'GET',
    ...
    expect(
      (
        await app.inject({
          method: 'GET',
          url: '/me',
          headers: { authorization: `Bearer ${enabled.token}` }
        })
      ).statusCode
    ).toBe(401);
    await app.close();
  ...
  it('creates independent concurrent sessions and revokes only the presented token', async () => {
    ...
    const leftToken = left.json().token as string;
    const rightToken = right.json().token as string;
    expect(leftToken).not.toBe(rightToken);
    await app.inject({
      method: 'POST',
      url: '/auth/logout',
      headers: { authorization: `Bearer ${leftToken}` }
    });
    expect(
      (
        await app.inject({
  ...
  it('uses only the bearer session identity and isolates two authenticated users', async () => {
    await createAccount('auth-user', 'employee@example.test');
    await createAccount('second-user', 'second@example.test');
    const app = await createApp({ prisma: db() });
    const first = (
      await app.inject({
        method: 'POST',
    ...
    const second = (
      await app.inject({
        method: 'POST',
        url: '/auth/login',
        payload: { login: 'second@example.test', password: 'DemoPassword!2026' }
      })
    ).json().token as string;
    const headers = {
    ...
    expect(
      (
        await app.inject({
          ...
          payload: { name: 'Forged', userId: 'second-user' }
        })
      ).statusCode
    ).toBe(400);
