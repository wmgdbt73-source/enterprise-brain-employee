...
async function fixture() {
  ...
  await c.projectMember.createMany({
    data: [
      {
        id: 'owner-project',
        projectId: 'project-a',
...
function input(
  ...
  const value = {
    ...
    taskId: 'task-a',
    agentId: 'agent-a',
    prompt: normalizeModelPrompt('  summarize this\ncarefully  '),
    idempotencyKey: 'model-key-a',
    ...overrides
  };
  return {
    ...value,
    requestFingerprint: modelRequestFingerprint({
      userId: value.userId,
      taskId: value.taskId,
...
describe('ModelInvocationRepository PostgreSQL integration', () => {
  ...
  it('creates one authorized MODEL AgentRun and RUNNING ModelInvocation', async () => {
    await fixture();
    const result = await new ModelInvocationRepository(db()).beginForTask(
      input()
    );
    ...
    expect(
      await db().agentRun.findUniqueOrThrow({ where: { id: 'run-a' } })
    ).toMatchObject({
      kind: 'MODEL',
      status: 'RUNNING',
      agentDefinitionKey: 'model-agent',
      agentVersion: 1
    });
  ...
  it('rejects unauthorized model runs without AgentRun or Invocation writes', async () => {
    await fixture();
    const repo = new ModelInvocationRepository(db());
    const cases = [
      ...
      async () => {
        ...
        return repo.beginForTask(input());
      },
      async () => {
    ...
    for (const attempt of cases) {
      await clean();
      await fixture();
      expect(typeof (await attempt())).toBe('string');
  ...
  it('reuses an idempotent run and rejects mismatched fingerprints', async () => {
    await fixture();
    const repo = new ModelInvocationRepository(db());
    const first = await repo.beginForTask(input());
    const second = await repo.beginForTask(
      input({ agentRunId: 'run-b', invocationId: 'invocation-b' })
    );
    if (typeof first === 'string' || typeof second === 'string')
      throw new Error('unexpected rejection');
    expect(second).toMatchObject({
      disposition: 'EXISTING_RUNNING',
      invocation: { id: first.invocation.id }
    });
    expect(
      await repo.beginForTask(
        input({
          agentRunId: 'run-c',
    ...
    await db().agentVersion.create({
      data: {
        ...
        agentDefinitionId: 'different-agent',
        version: 1,
        runtimeProfile: 'READ_ONLY_WORK',
        status: 'ACTIVE',
        createdAt: now
  ...
  it('coordinates concurrent same-key model run creation through the production repository', async () => {
    ...
    const second = createPrismaClient(connection);
                ...
                callback(
                  new Proxy(tx, {
                    get(inner, innerKey) {
                      ...
                      if (innerKey !== 'modelInvocation') return value;
    ...
    try {
      const [left, right] = await Promise.all([
        ...
        new ModelInvocationRepository(wrap(second)).beginForTask(
          input({
            agentRunId: 'run-concurrent',
            invocationId: 'invocation-concurrent'
          })
        )
      ]);
      if (typeof left === 'string' || typeof right === 'string')
        throw new Error(`unexpected concurrency outcome: ${left}/${right}`);
      expect(new Set([left.invocation.id, right.invocation.id]).size).toBe(1);
      expect(await db().modelInvocation.count()).toBe(1);
      expect(await db().agentRun.count()).toBe(1);
    } finally {
  ...
  it('completes or fails a model invocation exactly once and rolls back a failed Run transition', async () => {
    await fixture();
    const c = db();
    const repo = new ModelInvocationRepository(c);
    const started = await repo.beginForTask(input());
    ...
    expect(await repo.complete(complete)).toMatchObject({
      status: 'COMPLETED',
      outputText: 'answer',
      totalTokens: 3
    });
    expect(
      await c.agentRun.findUniqueOrThrow({ where: { id: 'run-a' } })
    ...
    const failure = {
      invocationId: failed.invocation.id,
      errorCode: 'MODEL_PROVIDER_TIMEOUT',
      completedAt: complete.completedAt
    };
    expect(await repo.fail(failure)).toMatchObject({
      status: 'FAILED',
      errorCode: 'MODEL_PROVIDER_TIMEOUT'
    });
    expect(
      await c.agentRun.findUniqueOrThrow({ where: { id: 'run-fail' } })
    ...
    expect(
      (
        await c.modelInvocation.findUniqueOrThrow({
          where: { id: failed.invocation.id }
        })
      ).outputText
    ).toBeNull();
            ...
            (tx: any) =>
              callback(
                new Proxy(tx, {
                  get(inner, innerKey) {
                    const value = Reflect.get(inner, innerKey);
                    if (innerKey !== 'agentRun') return value;
                    return new Proxy(value, {
                      get(model, method) {
    ...
    const rollback = await new ModelInvocationRepository(
      ...
      input({
        agentRunId: 'run-rollback',
        invocationId: 'invocation-rollback',
        idempotencyKey: 'rollback-key'
      })
    );
    if (typeof rollback === 'string') throw new Error(rollback);
    expect(
      await new ModelInvocationRepository(wrapped as typeof c).complete({
        invocationId: rollback.invocation.id,
        outputText: 'no',
        model: 'gpt-test',
        completedAt: complete.completedAt
      })
    ...
    expect(
      await c.modelInvocation.findUniqueOrThrow({
        where: { id: rollback.invocation.id }
      })
  ...
  it('lists only authorized Task model invocations newest first with a bounded limit', async () => {
    ...
    for (const [suffix, taskId] of [
      ...
      const value = await repo.beginForTask(
        input({
    ...
    const rows = await repo.listForTaskForMember({
      userId: 'member',
      taskId: 'task-a',
      limit: 1
    });
    expect(Array.isArray(rows) && rows).toHaveLength(1);
    expect(Array.isArray(rows) && rows[0]?.agentRunId).toBe('run-two');
    expect(
      await repo.listForTaskForMember({ userId: 'outsider', taskId: 'task-a' })
    ).toBe('NOT_FOUND');
  });
  ...
  it('persists ordered MODEL ToolCall provenance and enforces per-run identities', async () => {
    ...
    if (typeof first === 'string' || typeof second === 'string')
      throw new Error('unexpected begin rejection');
    const c = db();
    await c.agentToolCall.createMany({
      data: [
        {
          id: 'tool-1',
          agentRunId: 'run-a',
    ...
    expect(
      (
        await c.agentToolCall.findMany({
          where: { agentRunId: 'run-a' },
          orderBy: { sequence: 'asc' }
        })
      ).map((row) => row.sequence)
    ).toEqual([1, 2, 3]);
    await expect(
      c.agentToolCall.create({
        data: {
    ...
    expect(
      await c.agentToolCall.findUniqueOrThrow({
        where: { id: 'legacy-tool-call' }
      })
    ).toMatchObject({
      agentRunId: 'legacy-tool-run',
      providerCallId: null,
  ...
  it('rolls back a failed read-tool transaction without returning tool data', async () => {
    await fixture();
    const failing = toolWriteFailurePrisma();
    const provider = new FixedToolProvider();
    const app = await createApp({ prisma: failing, modelProvider: provider });
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/tasks/task-a/agent-responses',
      headers: {
        authorization: `Bearer ${token}`,
        'idempotency-key': 'tool-write-failure'
      },
      payload: { agentId: 'agent-a', prompt: 'read safely' }
    });
    expect(response.statusCode).toBe(502);
    expect(JSON.stringify(response.json())).not.toContain(
      'forced tool provenance failure'
    );
    ...
    expect(await toolTransactionBusinessState()).toEqual(before);
    await app.close();
  });
});
...
class FixedToolProvider implements ModelProvider {
  ...
  async generate(input: ModelGenerationRequest): Promise<ModelGeneration> {
...
async function toolTransactionBusinessState() {
  const c = db();
  return {
    task: await c.task.findMany({ orderBy: { id: 'asc' } }),
    dependencies: await c.taskDependency.findMany(),
    artifacts: await c.artifact.findMany(),
    results: await c.result.findMany(),
    reviews: await c.review.findMany()
  };
}
function toolWriteFailurePrisma() {
  const base = db();
            ...
            callback(
              new Proxy(tx, {
                get(
                  ...
                  if (transactionProperty !== 'agentToolCall') {
                  ...
                  return new Proxy(tx.agentToolCall, {
                    get(delegate, delegateProperty, delegateReceiver) {
                      if (delegateProperty === 'create')
                        return async () => {
                          throw new Error('forced tool provenance failure');
                        };
                      const value = Reflect.get(
                        delegate,
                        delegateProperty,
