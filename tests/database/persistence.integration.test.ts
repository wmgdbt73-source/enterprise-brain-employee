...
describe('PostgreSQL persistence constraints', () => {
  ...
  it('enforces organization and department membership composite constraints', async () => {
    ...
    await expect(
      db.departmentMembership.create({
        data: {
          id: 'dm-duplicate',
          organizationId: 'org-a',
          departmentId: 'dept-a',
    ...
    ).rejects.toMatchObject({ code: expect.stringMatching(/^P2003$|^P2039$/) });
    await expect(
      db.departmentMembership.create({
        data: {
          id: 'dm-wrong-membership',
          organizationId: 'org-a',
          departmentId: 'dept-a',
          userId: 'org-user-d',
          role: 'MEMBER',
  ...
  it('enforces PermissionOverride ownership, tuple uniqueness, and a nonblank scope', async () => {
    ...
    await db.organizationMembership.createMany({
      data: [
        {
          id: 'permission-om-a',
          organizationId: 'permission-org-a',
          userId: 'permission-user-a',
          role: 'MEMBER',
          status: 'ACTIVE',
          createdAt: now,
          updatedAt: now
        ...
        {
          id: 'permission-om-b',
          organizationId: 'permission-org-b',
          userId: 'permission-user-b',
          role: 'MEMBER',
          status: 'ACTIVE',
          createdAt: now,
          updatedAt: now
    ...
    await db.permissionOverride.create({
      data: { id: 'permission-one', ...row }
    });
    await expect(
      db.permissionOverride.create({ data: { id: 'permission-two', ...row } })
    ).rejects.toMatchObject({ code: 'P2002' });
    await expect(
      db.permissionOverride.create({
        data: { id: 'permission-cross', ...row, userId: 'permission-user-b' }
      })
    ).rejects.toMatchObject({ code: expect.stringMatching(/^P2003$|^P2039$/) });
    await expect(
      db.permissionOverride.create({
        data: { id: 'permission-blank', ...row, scopeId: '   ' }
      })
    ).rejects.toMatchObject({
      code: expect.stringMatching(/^P2003$|^P2010$|^P2039$/)
    });
  });
  ...
  it('rolls back a failed transactional department assignment without replacing the previous assignment', async () => {
    ...
    await db.organizationMembership.createMany({
      data: [
        ...
        {
          id: 'assign-om-employee',
          organizationId: 'assign-org',
          userId: 'assign-employee',
          role: 'MEMBER',
          status: 'ACTIVE',
          createdAt: now,
          updatedAt: now
    ...
    await expect(
      new OrganizationRepository(db).assign(
        'assign-owner',
        'assign-employee',
        'assign-new',
        'INVALID' as never
  ...
  it('enforces Account and hashed Session persistence boundaries', async () => {
    ...
    const rawToken = 'opaque-demo-token';
    ...
    expect(
      (await db.session.findUniqueOrThrow({ where: { id: 'session-1' } }))
        .tokenHash
    ).not.toBe(rawToken);
    await expect(
      db.session.create({
    ...
    await expect(
      db.account.create({
        data: {
          id: 'account-uppercase',
          userId: 'user-canonical-check',
          login: 'OWNER@EXAMPLE.TEST',
          passwordHash,
          status: 'ACTIVE',
          createdAt: now,
  ...
  it('enforces ResultArtifact composite provenance and rolls back incomplete composition', async () => {
    ...
    await db.resultArtifact.create({
      data: {
        resultId: result.id,
    ...
    await db.agentRun.create({
      data: {
        ...
        taskId: 'task-3',
        agentDefinitionKey: 'read-only-work-agent-v1',
        intent: { name: 'read_file', relativePath: '3.md' },
        status: 'SUCCEEDED',
        createdAt: now,
        startedAt: now,
        finishedAt: now,
    ...
    await db.artifact.create({
      data: {
        ...
        taskId: 'task-3',
        agentRunId: 'result-run-3',
        sourceToolCallId: 'result-call-3',
        type: 'FILE',
    ...
    await expect(
      db.result.create({
        data: resultRow('forged-result', 'task-3', 'project-1', now)
      })
    ).rejects.toMatchObject({ code: 'P2003' });
    await expect(
      db.resultArtifact.create({
    ...
    const repository = new ResultRepository(
      wrapPrismaForResultTests(db, {
        beforeResultArtifactCreateMany: async () => {
          throw new Error('inject link creation failure');
        }
      })
    );
    await expect(
      repository.createForTaskForUser({
        resultId: 'rolled-back-result',
        taskId: 'task-1',
        userId: 'user-owner',
        artifactIds: ['artifact-1'],
        idempotencyKey: 'rolled-back-result-key',
        now
      })
    ).rejects.toThrow('inject link creation failure');
    expect(
      await db.result.findUnique({ where: { id: 'rolled-back-result' } })
    ).toBeNull();
    expect(
      await db.resultArtifact.count({
        where: { resultId: 'rolled-back-result' }
      })
    ).toBe(0);
  });
  ...
  it('keeps ResultRepository scoped reads on one RepeatableRead snapshot', async () => {
    ...
    await db.result.create({
      data: resultRow('snapshot-result', 'task-1', 'project-1', now)
    });
    await db.resultArtifact.create({
      data: {
        resultId: 'snapshot-result',
        artifactId: 'artifact-1',
        taskId: 'task-1',
        projectId: 'project-1'
      }
  ...
  it('coordinates real ResultRepository transactions before insertion for idempotent concurrency', async () => {
    ...
    const sameRepository = new ResultRepository(
      wrapPrismaForResultTests(leftClient, {
        beforeResultCreate: () => sameGate.arriveAndWait()
      })
    );
    const sameRightRepository = new ResultRepository(
      wrapPrismaForResultTests(rightClient, {
        beforeResultCreate: () => sameGate.arriveAndWait()
      })
    );
    const sameInput = (resultId: string) => ({
      resultId,
      taskId: 'task-1',
      userId: 'user-owner',
      artifactIds: ['artifact-1'],
    ...
    const sameLeft = sameRepository.createForTaskForUser(
      sameInput('same-left')
    );
    const sameRight = sameRightRepository.createForTaskForUser(
      sameInput('same-right')
    );
    ...
    try {
      ...
      sameResults = same.filter(
        (value): value is Exclude<typeof value, string> =>
          typeof value !== 'string'
      );
      expect(new Set(sameResults.map((value) => value.result.id)).size).toBe(1);
      expect(sameResults.map((value) => value.created).sort()).toEqual([
        false,
        true
      ]);
      expect(
        await db.resultArtifact.findMany({
          where: { resultId: sameResults[0].result.id },
    ...
    const conflictRepository = new ResultRepository(
      wrapPrismaForResultTests(leftClient, {
        beforeResultCreate: () => conflictGate.arriveAndWait()
      })
    );
    const conflictRightRepository = new ResultRepository(
      wrapPrismaForResultTests(rightClient, {
        beforeResultCreate: () => conflictGate.arriveAndWait()
      })
    );
    const conflictLeft = conflictRepository.createForTaskForUser({
      resultId: 'conflict-left',
      taskId: 'task-1',
      userId: 'user-owner',
      artifactIds: ['artifact-1'],
    ...
    const conflictRight = conflictRightRepository.createForTaskForUser({
      resultId: 'conflict-right',
      taskId: 'task-1',
      userId: 'user-owner',
    ...
    try {
      ...
      expect(
        conflict.filter((value) => value === 'IDEMPOTENCY_CONFLICT')
      ).toHaveLength(1);
      const winner = conflict.find(
        (value): value is Exclude<typeof value, string> =>
          typeof value !== 'string'
      );
      expect(winner).toBeDefined();
      const winnerLinks = await db.resultArtifact.findMany({
        where: { resultId: winner!.result.id },
        orderBy: { artifactId: 'asc' }
      });
      expect(winnerLinks.map((link) => link.artifactId)).toEqual(
        winner!.result.artifactIds
      );
      expect(
        await db.result.count({ where: { idempotencyKey: 'conflict-key' } })
      ).toBe(1);
    } finally {
  ...
  it('returns NOT_FOUND when membership is revoked before real idempotency conflict recovery', async () => {
    ...
    const retry = repository.createForTaskForUser({
      resultId: 'recovery-retry',
      taskId: 'task-1',
      userId: 'user-owner',
      artifactIds: ['artifact-1'],
    ...
    try {
      ...
      await db.projectMember.delete({
        where: {
          projectId_userId: { projectId: 'project-1', userId: 'user-owner' }
        }
      });
  ...
  it('returns NOT_FOUND when membership is revoked before submit-for-review recovery', async () => {
    ...
    await db.result.create({
      data: resultRow('submit-recovery', 'task-1', 'project-1', now)
    });
    await db.resultArtifact.create({
      data: {
        resultId: 'submit-recovery',
        artifactId: 'artifact-1',
        taskId: 'task-1',
        projectId: 'project-1'
      }
      ...
      wrapPrismaForResultTests(db, {
        beforeSubmissionResultUpdate: async () => {
          ...
          await db.$transaction(async (other) => {
            await other.result.update({
              where: { id: 'submit-recovery' },
              data: {
                status: 'HUMAN_REVIEW',
                submittedByUserId: 'user-owner',
                submittedAt: now,
                updatedAt: now
              }
            });
            await other.projectMember.delete({
              where: {
                projectId_userId: {
                  projectId: 'project-1',
                  userId: 'user-owner'
    ...
    await expect(
      repository.submitForReviewForCreator('submit-recovery', 'user-owner', now)
  ...
  it('coordinates concurrent Candidate submissions at the production Task CAS boundary', async () => {
    ...
    for (const [id, artifactId] of [
      ['submit-left', 'artifact-1'],
      ['submit-right', 'artifact-4']
    ] as const) {
      await db.result.create({
        data: resultRow(id, 'task-1', 'project-1', now)
      });
      await db.resultArtifact.create({
        data: {
          resultId: id,
    ...
    const left = new ResultRepository(
      wrapPrismaForResultTests(leftClient, {
        beforeTaskUpdate: () => gate.arriveAndWait()
      })
    );
    const right = new ResultRepository(
      wrapPrismaForResultTests(rightClient, {
        beforeTaskUpdate: () => gate.arriveAndWait()
      })
    );
    const leftSubmit = left.submitForReviewForCreator(
      'submit-left',
      'user-owner',
      now
    );
    const rightSubmit = right.submitForReviewForCreator(
      'submit-right',
      'user-owner',
      now
    );
    ...
    try {
      ...
      expect(
        outcomes.filter((value) => value === 'INVALID_STATE')
      ).toHaveLength(1);
      expect(
        outcomes.filter((value) => typeof value !== 'string')
      ).toHaveLength(1);
      expect(
        await db.task.findUniqueOrThrow({ where: { id: 'task-1' } })
      ).toMatchObject({ status: 'READY_FOR_REVIEW' });
      const submitted = await db.result.findMany({
        where: { id: { in: ['submit-left', 'submit-right'] } },
        orderBy: { id: 'asc' }
      });
      expect(submitted.map((result) => result.status).sort()).toEqual([
        'CANDIDATE',
        'HUMAN_REVIEW'
      ]);
    } finally {
  ...
  it('rolls back submission and review when the production Task CAS write fails', async () => {
    ...
    await db.result.create({
      data: resultRow('submit-task-rollback', 'task-1', 'project-1', now)
    });
    await db.resultArtifact.create({
      data: {
        resultId: 'submit-task-rollback',
        artifactId: 'artifact-1',
        taskId: 'task-1',
        projectId: 'project-1'
      }
    });
    const failingSubmit = new ResultRepository(
      wrapPrismaForResultTests(db, {
        beforeTaskUpdate: () => {
          throw new Error('inject submission Task write failure');
        }
      })
    );
    await expect(
      failingSubmit.submitForReviewForCreator(
        'submit-task-rollback',
        'user-owner',
        now
      )
    ).rejects.toThrow('inject submission Task write failure');
    expect(
      await db.result.findUniqueOrThrow({
        where: { id: 'submit-task-rollback' }
      })
    ).toMatchObject({
      status: 'CANDIDATE',
    ...
    const failingReview = new ResultRepository(
      wrapPrismaForResultTests(db, {
        beforeTaskUpdate: () => {
          throw new Error('inject review Task write failure');
        }
      })
    );
    await expect(
      failingReview.decideForReviewer({
        resultId: 'review-task-rollback',
        reviewerId: 'reviewer-1',
        decision: 'ACCEPT',
        reviewId: 'review-task-failed',
        now
      })
    ).rejects.toThrow('inject review Task write failure');
    expect(
      await db.result.findUniqueOrThrow({
        where: { id: 'review-task-rollback' }
      })
    ).toMatchObject({ status: 'HUMAN_REVIEW' });
    expect(
      await db.task.findUniqueOrThrow({ where: { id: 'task-1' } })
    ).toMatchObject({ status: 'READY_FOR_REVIEW' });
    expect(
      await db.review.count({ where: { resultId: 'review-task-rollback' } })
    ).toBe(0);
  });
  ...
  it('coordinates real reviewer transactions and rolls back a failed Review creation', async () => {
    ...
    const left = new ResultRepository(
      wrapPrismaForResultTests(leftClient, {
        beforeReviewResultUpdate: () => gate.arriveAndWait()
      })
    );
    const right = new ResultRepository(
      wrapPrismaForResultTests(rightClient, {
    ...
    const accepted = left.decideForReviewer({
      resultId: 'review-race',
      reviewerId: 'reviewer-1',
      decision: 'ACCEPT',
      reviewId: 'review-accept',
      now
    });
    const reworked = right.decideForReviewer({
      resultId: 'review-race',
    ...
    try {
      ...
      expect(
        decisions.filter((decision) => decision === 'CONFLICT')
      ).toHaveLength(1);
      const winner = decisions.find(
        (decision): decision is Exclude<typeof decision, string> =>
          typeof decision !== 'string'
      );
      expect(winner?.created).toBe(true);
      const persisted = await db.result.findUniqueOrThrow({
        where: { id: 'review-race' }
      });
      const reviews = await db.review.findMany({
        where: { resultId: 'review-race' }
      });
      expect(reviews).toHaveLength(1);
      expect(persisted.status).toBe(
        reviews[0].decision === 'ACCEPT' ? 'ACCEPTED' : 'REWORK'
      );
      expect(
        (await db.task.findUniqueOrThrow({ where: { id: 'task-1' } })).status
      ).toBe(reviews[0].decision === 'ACCEPT' ? 'ACCEPTED' : 'IN_PROGRESS');
    } finally {
      gate.cleanup();
      await leftClient.$disconnect();
      await rightClient.$disconnect();
    ...
    const sameGate = boundedGate(2);
    const sameLeft = new ResultRepository(
      wrapPrismaForResultTests(sameLeftClient, {
        beforeReviewResultUpdate: () => sameGate.arriveAndWait()
      })
    );
    const sameRight = new ResultRepository(
      wrapPrismaForResultTests(sameRightClient, {
        beforeReviewResultUpdate: () => sameGate.arriveAndWait()
      })
    );
    const input = (reviewId: string) => ({
      resultId: 'review-same',
      reviewerId: 'reviewer-1',
    ...
    try {
      ...
      expect(
        outcomes
          .map((outcome) =>
            typeof outcome === 'string' ? outcome : outcome.created
          )
          .sort()
      ).toEqual([false, true]);
      const reviews = await db.review.findMany({
        where: { resultId: 'review-same' }
      });
      expect(reviews).toHaveLength(1);
    ...
    const failing = new ResultRepository(
      wrapPrismaForResultTests(db, {
        beforeReviewCreate: () => {
          throw new Error('inject Review creation failure');
        }
      })
    );
    await expect(
      failing.decideForReviewer({
        resultId: 'review-rollback',
        reviewerId: 'reviewer-1',
        decision: 'ACCEPT',
        reviewId: 'review-failed',
        now
      })
    ).rejects.toThrow('inject Review creation failure');
    expect(
      await db.result.findUniqueOrThrow({ where: { id: 'review-rollback' } })
    ).toMatchObject({ status: 'HUMAN_REVIEW' });
    expect(
      await db.task.findUniqueOrThrow({ where: { id: 'task-1' } })
    ).toMatchObject({ status: 'READY_FOR_REVIEW' });
    expect(
      await db.review.count({ where: { resultId: 'review-rollback' } })
    ).toBe(0);
  });
  ...
  it('rejects a deferred write ToolCall creation without its pending confirmation', async () => {
    ...
    expect(
      await db.agentRun.findUnique({ where: { id: 'write-run' } })
    ).toBeNull();
    expect(
      await db.agentToolCall.findUnique({ where: { id: 'write-call' } })
    ).toBeNull();
  });
  ...
  it('allows approved write ToolCalls to finish after the insert-only deferred trigger', async () => {
    ...
    expect(
      await db.agentToolCall.findUniqueOrThrow({ where: { id: 'write-call' } })
  ...
  it('rolls back all write rows when confirmation creation fails and rejects invalid device provenance', async () => {
    ...
    await expect(
      db.$transaction(async (tx) => {
        await tx.agentRun.create({ data: writeRun(now) });
        await tx.agentToolCall.create({ data: writeCall(now) });
        await tx.humanConfirmation.create({
          data: { ...writeConfirmation(now), deviceId: '' }
        });
      })
    ).rejects.toThrow();
    expect(
    ...
    expect(
      await db.humanConfirmation.findUnique({
        where: { id: 'write-confirmation' }
      })
    ).toBeNull();
    ...
    await expect(
      db.humanConfirmation.update({
        where: { id: 'write-confirmation' },
        data: { deviceId: 'other-device' }
      })
    ).rejects.toThrow();
    await expect(
      db.humanConfirmation.update({
    ...
    await expect(
      db.agentToolCall.create({
        data: { ...writeCall(now), id: 'blank-device-call', deviceId: '' }
      })
    ).rejects.toThrow();
  });
  ...
  it('enforces immutable one-per-Result Human Review provenance', async () => {
    ...
    await db.result.create({
      data: {
        ...resultRow('review-result', 'task-1', 'project-1', now),
        status: 'HUMAN_REVIEW',
        submittedByUserId: 'user-owner',
        submittedAt: now
      }
    ...
    await db.result.create({
      data: {
        ...resultRow('review-result-2', 'task-1', 'project-1', now),
        status: 'HUMAN_REVIEW',
        submittedByUserId: 'user-owner',
        submittedAt: now
      }
    ...
    await expect(
      db.review.create({
        data: {
          id: 'review-project-mismatch',
          resultId: 'review-result-2',
          projectId: 'missing-project',
          reviewerId: 'reviewer-1',
          decision: 'ACCEPT',
          reviewedAt: now
    ...
    await expect(
      db.review.create({
        data: {
          id: 'review-user-mismatch',
          resultId: 'review-result-2',
          projectId: 'project-1',
          reviewerId: 'missing-user',
          decision: 'ACCEPT',
          reviewedAt: now
        }
      })
...
async function createResultPersistenceFixture(
  ...
  for (const suffix of ['1', '2']) {
    ...
    await db.agentRun.create({
      data: {
        id: `result-run-${suffix}`,
        userId: 'user-owner',
        projectId: 'project-1',
        taskId,
        agentDefinitionKey: 'read-only-work-agent-v1',
        intent: { name: 'read_file', relativePath: `${suffix}.md` },
        status: 'SUCCEEDED',
        createdAt: now,
        startedAt: now,
        finishedAt: now,
    ...
    await db.agentToolCall.create({
      data: {
        id: `result-call-${suffix}`,
        agentRunId: `result-run-${suffix}`,
        sequence: 1,
        name: 'read_file',
        request: {
          id: `result-call-${suffix}`,
          runId: `result-run-${suffix}`,
          userId: 'user-owner',
          projectId: 'project-1',
          name: 'read_file',
          relativePath: `${suffix}.md`
        },
        status: 'SUCCEEDED',
        receipt: {
          toolCallId: `result-call-${suffix}`,
          status: 'SUCCEEDED',
          metadata: {
            relativePath: `${suffix}.md`,
            size: 1,
    ...
    await db.artifact.create({
      data: {
        ...
        taskId,
        agentRunId: `result-run-${suffix}`,
        sourceToolCallId: `result-call-${suffix}`,
        type: 'FILE',
        storageKind: 'LOCAL_WORKSPACE',
        relativePath: `${suffix}.md`,
        size: 1,
  ...
  await db.agentRun.create({
    data: {
      ...
      taskId: 'task-1',
      agentDefinitionKey: 'read-only-work-agent-v1',
      intent: { name: 'read_file', relativePath: '4.md' },
      status: 'SUCCEEDED',
      createdAt: now,
      startedAt: now,
      finishedAt: now,
  ...
  await db.artifact.create({
    data: {
      ...
      taskId: 'task-1',
      agentRunId: 'result-run-4',
      sourceToolCallId: 'result-call-4',
      type: 'FILE',
      storageKind: 'LOCAL_WORKSPACE',
      relativePath: '4.md',
      size: 1,
...
async function createHumanReviewFixture(
  db: ReturnType<typeof requireDatabase>,
  now: Date,
  id: string
) {
  ...
  for (const reviewerId of ['reviewer-1', 'reviewer-2']) {
    ...
    await db.projectMember.upsert({
      where: {
        projectId_userId: { projectId: 'project-1', userId: reviewerId }
      },
      create: {
    ...
    await db.organizationMembership.upsert({
      ...
      create: {
        id: `review-organization-${reviewerId}`,
  ...
  await db.result.create({
    data: {
      ...resultRow(id, 'task-1', 'project-1', now),
      status: 'HUMAN_REVIEW',
      submittedByUserId: 'user-owner',
      submittedAt: now
    }
...
async function createWriteTask(
  db: ReturnType<typeof requireDatabase>,
  now: Date
) {
  await db.task.create({
...
function writeRun(now: Date) {
  return {
    id: 'write-run',
    userId: 'user-owner',
    projectId: 'project-1',
    taskId: 'write-task',
    agentDefinitionKey: 'confirmed-write-work-agent-v1',
    intent: { name: 'write_file', relativePath: 'a.md' },
    status: 'WAITING_HUMAN' as const,
    createdAt: now,
    updatedAt: now
  };
}
function writeCall(now: Date) {
  return {
    id: 'write-call',
    agentRunId: 'write-run',
    sequence: 1,
    name: 'write_file',
    deviceId: 'device-1',
    request: {
...
function writeConfirmation(now: Date) {
  return {
              ...
              get(target, property, receiver) {
                ...
                return new Proxy(model, {
                  get(resultTarget, resultProperty, resultReceiver) {
                    ...
                    if (
                      property === 'result' &&
                      resultProperty === 'findFirst'
                    ) {
                      return async (...argumentsList: unknown[]) => {
                        if (transactionNumber === 2)
                          await hooks.beforeRecoveryResultFindFirst?.();
...
function boundedGate(parties: number, timeoutMs = 5_000) {
  ...
  const timeout = setTimeout(
    () =>
      failGate(
        new Error(
          `timed out waiting for ${parties} repository transaction boundaries`
        )
      ),
    timeoutMs
  );
