...
export class SessionRepository {
  ...
  async login(loginInput: string, password: string, now = new Date()): Promise<{ token: string; user: CurrentUserContract } | undefined> {
    ...
    const account = await this.prisma.account.findUnique({ where: { login }, include: { user: true } });
  ...
  async resolveBearer(value: string | undefined, now = new Date()): Promise<CurrentUserContract | undefined> {
    ...
    const session = await this.prisma.session.findUnique({ where: { tokenHash: hashSessionToken(token) }, include: { account: { include: { user: true } } } });
...
function toCurrentUser(user: { id: string; name: string; systemRole: 'EMPLOYEE' | 'ADMIN' }): CurrentUserContract {
  return { id: user.id as CurrentUserContract['id'], name: user.name, systemRole: user.systemRole };
}
