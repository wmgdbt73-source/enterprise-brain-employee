...
import { DomainError } from '@enterprise-brain/domain';
...
export class ResultService {
  ...
  async create(context: RequestContext, taskId: string, artifactIds: string[]): Promise<ResultContract> { if (artifactIds.length === 0 || new Set(artifactIds).size !== artifactIds.length) throw new DomainError('INVALID_ARGUMENT', 'Result requires unique Artifact IDs'); const result = await this.results.createForMember({ id: randomUUID(), taskId, userId: context.currentUser.id, artifactIds, now: new Date() }); if (result === 'NOT_FOUND') throw new ResultNotFoundError(); return result; }
