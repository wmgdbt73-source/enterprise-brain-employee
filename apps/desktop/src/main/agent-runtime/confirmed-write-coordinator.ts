...
export class ConfirmedWriteCoordinator {
  ...
  async prepare(taskId: string, input: WriteInput) {
    try {
      ...
      const available = await this.gateway.listAvailableAgents(); if (!available.ok) return available;
      const agent = available.data.find(value => value.runtimeProfile === 'CONFIRMED_WRITE_WORK'); if (!agent) return fail('AGENT_TOOL_NOT_ALLOWED', 'No confirmed-write Agent is available');
      const created = await this.gateway.createAgentRun(taskId, agent.id, { name: 'write_file', relativePath: prepared.relativePath, payloadSize: prepared.payloadSize, payloadSha256: prepared.payloadSha256, effect: prepared.effect, ...(prepared.expectedCurrentSha256 ? { expectedCurrentSha256: prepared.expectedCurrentSha256 } : {}), deviceId: context.deviceId });
